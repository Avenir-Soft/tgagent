...
async def process_dm_message(
    ...
    try:
        ...
        # --- Step 2: Load AI settings ---
        from src.ai.models import AiSettings
        ai_s_result = await db.execute(select(AiSettings).where(AiSettings.tenant_id == tenant_id))
        ai_settings = ai_s_result.scalar_one_or_none()
        ...
        if ai_settings and ai_settings.auto_handoff_on_profanity and contains_profanity(user_message):
            return await _handle_profanity(
